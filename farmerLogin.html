	...
	<div class="dashboard container-box">
		...
		<div class="dashboardItems " id="dashboard" style="display:block">
			...
			<div class="dashboard-item-details">
				<div class="info">
					<span class="name"><?php echo $farmerRow['fname']; ?></span>
					<p><?php echo $farmerRow['email']; ?></p>
					<p><?php echo $farmerRow['mno']; ?></p>
					<p><?php echo $farmerRow['city']; ?></p>
		...
		<div class="dashboardItems" id="crops">
			...
			<table border="0" cellspacing="0" cellpadding="45px">
					...
					<tbody>
			<?php
				$cropDetail="SELECT * FROM crops where fid=(SELECT fid FROM farmer_id where email='".$_SESSION["email"]."')";
				$cropDetailResult=mysqli_query($connection,$cropDetail);
				$cropCheck = mysqli_num_rows($cropDetailResult) > 0;
				
				if($cropCheck){
					while($cropRow=mysqli_fetch_array($cropDetailResult)){
			?>
						<tr>
							<td><img src="images/crops/<?php echo $cropRow['crop_img'] ?>"></td>
							<td><?php echo $cropRow['crop_name'] ?></td>
							<td><?php echo $cropRow['crop_ctg'] ?></td>
							<td><?php echo $cropRow['price'] ?></td>
							<td><?php echo $cropRow['quantity'] ?>KG</td>
							<td><?php echo $cropRow['date'] ?></td>
			...
			<?php
					...
					echo '<p style="color:red; font-weight:600; font-size:14px">'.$farmerRow['fname'].' has no Crops</p>';
				}
